...
def run_one(afm, truth_col, cells_all, shipped=None, clone_sets=None, tag=None):
    ...
    info = {}
    # fix 1: signal-to-background filter on the binomial genotype calls
    ok_snr, ratio = signal_to_background(AD[:, cols], X[:, cols], p0, min_ratio=10.0)
    dropped = pd.DataFrame(dict(var=np.array(c.var_names)[cols][~ok_snr], ratio=np.round(ratio[~ok_snr], 2),
                                background=p0[~ok_snr], calls=(Bgen[:, ~ok_snr] > 0).sum(0)))
    snr_drop_log.append(dropped.assign(**(tag or {})))
    cols_s = cols[ok_snr]; Bgen_s = Bgen[:, ok_snr]; p0_s = p0[ok_snr]
    lab_s, lab_st, fc_s, Bs_s = tree_cut(cols_s, Bgen_s)
    labels['QC 1e-4 + binom 1e-3 + SNR>=10'] = lab_s
    labels['QC 1e-4 + binom 1e-3 + SNR>=10 + tau'] = lab_st
    if fc_s.size:
        r, det = pooled_rescue(lab_st, Bgen_s[:, fc_s] > 0, AD[:, cols_s[fc_s]], COV[:, cols_s[fc_s]], p0_s[fc_s], alpha=0.01)
        labels['QC 1e-4 + binom 1e-3 + SNR>=10 + tau + pooled rescue'] = r
        info['QC 1e-4 + binom 1e-3 + SNR>=10 + tau + pooled rescue'] = int(det.assigned.sum()) if len(det) else 0
...
if mode == 'real':
    ...
    R = pd.DataFrame(rows); R.to_csv(f'snr_{ds}.csv', index=False)
    pd.concat(snr_drop_log).to_csv(f'snr_dropped_{ds}.csv', index=False)
    print('dropped by SNR filter:'); print(pd.concat(snr_drop_log).to_string(index=False))
    ...
    for i, (nk, topo, seed) in enumerate(configs):
        ...
        pd.DataFrame(rows).to_csv(f'snr_sim_{shard}.csv', index=False)
        pd.concat(snr_drop_log).to_csv(f'snr_dropped_sim_{shard}.csv', index=False)
