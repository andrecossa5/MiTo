...
for i, (nk, topo, seed) in enumerate(configs):
    ...
    for qc_name, Bq in [('guard flat', (g_flat > 0.7).astype(np.int8)), ('AD>=2', (AD >= 2).astype(np.int8)), ('AD>=1', (AD >= 1).astype(np.int8))]:
        ...
        for r in cluster(c, cols_f, Bs, Xs, Xk, 'clone', 1000):
            rows.append({**front, **r})
    pd.DataFrame(rows).to_csv(f'adqc_sim_{shard}.csv', index=False)
