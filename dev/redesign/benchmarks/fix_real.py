...
keep, pj, pe = carrier_nonrandomness(X, Bf, cell_depth=COV.mean(1), alpha=0.05)
cols = np.flatnonzero(keep)
print(f'QC keeps {cols.size} ({sum(v in GT for v in c.var_names[cols])} GT of {len(GT)})', flush=True)
for split in (False, True):
    ...
    name = f'QC2 a=0.05 guard {"split" if split else "no split"}'
    ...
    pd.DataFrame(rows).to_csv(f'fix_real_{ds}.csv', index=False)
