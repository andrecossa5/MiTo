...
def run_one(afm, truth_col, shipped=None, tag=None, per_clone=False):
    ...
    for a_geno in (1e-3, 1e-2):
        ...
        for imp in (False, True):
            Bk = impute_dropouts(X[:, cs], Bg, k=30, thr=0.6, min_support=1)[0] if imp else Bg
            for tau in (True, False):
                lab, fc, Bs, n_call, n_cut = tree_cut(cs, Bk, tau=tau)
                nm = f'geno {a_geno:g}{" + imp" if imp else ""}{" + tau" if tau else ""}'
...
if mode == 'real':
    ...
    R = pd.DataFrame(rows); R.to_csv(f'coverage_{ds}.csv', index=False)
    ...
    for i, (nk, topo, seed) in enumerate(configs):
        ...
        pd.DataFrame(rows).to_csv(f'coverage_sim_{shard}.csv', index=False)
